...
fn test_query_user_debt_v2() {
    ...
    let res = DEBTS.load(&deps.storage, (&rb_user_id_key, "coin_2")).unwrap();

    println!("🔥res: {:?}", res);

    // Credit manager user: Check asset with existing debt
    ...
    // Credit manager user:  Check asset with no debt
    let res = query_user_debt_v2(
        deps.as_ref(),
        &env.block,
        cm_user.clone(),
    ...
    // RedBank user: Check asset with existing debt
    let res = query_user_debt_v2(
        deps.as_ref(),
        &env.block,
        rb_user.clone(),
