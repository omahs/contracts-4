...
fn cannot_borrow_more_but_not_liquidatable() {
    ...
    let mut mock = MockEnv::new()
        ...
        .fund_account(AccountToFund {
            addr: user.clone(),
            funds: coins(3000000, uosmo_info.denom.clone()),
        })
        .build()
        .unwrap();
    let account_id = mock.create_credit_account(&user).unwrap();
    ...
    mock.update_credit_account(
        &account_id,
        &user,
        vec![Deposit(uosmo_info.to_coin(3000000)), Borrow(uatom_info.to_coin(500000))],
        &[Coin::new(3000000, uosmo_info.denom)],
    )
    .unwrap();

    let health = mock.query_health(&account_id, AccountKind::Default, ActionKind::Default);
    assert!(!healt.liquidatable);
    assert!(!health.above_max_ltv);
...
fn debt_value() {
    ...
    // TODO: Piotr -> Add simulate iunterest. Is this fine?
